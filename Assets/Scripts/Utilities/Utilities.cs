...
namespace Utils
{
	partial class Utilities
	{
        ...
        public static float GetAngleTurnToFace(float _angleDegTarget, float _angleDegCurrent, float _angleDegThreshold, float _rotationDegPerSecond)
        {
            ...
            rotation = Time.deltaTime * _rotationDegPerSecond;


            _angleDegTarget += 450;
            _angleDegTarget = _angleDegTarget % 360;
            if (_angleDegTarget > 360) { _angleDegTarget -= 360; }
            diff = Mathf.Abs(_angleDegCurrent - _angleDegTarget);

            
            if (diff < _angleDegThreshold)
                rv = 0;
            else if (_angleDegCurrent == _angleDegTarget)
                rv = 0;
            else if (_angleDegCurrent > _angleDegTarget)
            {
                if (diff < rotation)
