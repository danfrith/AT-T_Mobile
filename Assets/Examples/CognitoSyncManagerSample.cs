...
namespace AWSSDK.Examples
{
    ...
    public class CognitoSyncManagerSample : MonoBehaviour
    {
        ...
        void Start()
        {
			...
			Debug.Log ("datasets completed");


            // Fetch locally stored data from a previous run
            alias = string.IsNullOrEmpty(playerInfo.Get("alias")) ? "MobileUser01" : playerInfo.Get("alias");
            playerName = string.IsNullOrEmpty(playerInfo.Get("playerName")) ? "TestName" : playerInfo.Get("playerName");
