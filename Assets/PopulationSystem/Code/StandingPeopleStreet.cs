...
public class StandingPeopleStreet : MonoBehaviour {
	...
	{	
		...
		for(int i = 0; i < one; i++)
		{
			...
			{
				var obj = Instantiate(peoplePrefabs[prefabNum], randomPosition, Quaternion.identity) as GameObject;

			    RaycastHit hit;

        		if(Physics.Raycast(obj.transform.position + new Vector3(0, 2, 0), -obj.transform.up, out hit))
        		{
		...
		for(int i = 0; i < two / 2; i++)
		{
			...
			if(randomPosition != Vector3.zero)
			{
				...
				if(randomPos1 != Vector3.zero && randomPos2 != Vector3.zero)
				{
        			...
        			if(Physics.Raycast(obs.transform.position + new Vector3(0, 2, 0), -obs.transform.up, out hit1))
        			{
		...
		for(int i = 0; i < three / 3; i++)
		{
			...
			if(randomPosition != Vector3.zero)
			{
        		...
        		if(Physics.Raycast(obl.transform.position + new Vector3(0, 2, 0), -obl.transform.up, out hit2))
        		{
